...
pane.addInput(paramter, 'culling').on('change', (v) => {
  app.setCulling(v.value)
})
// Activate depth test
pane.addInput(paramter, 'depthTest').on('change', (v) => {
  app.setDepthTest(v.value)
})
// Activate rotation
pane.addInput(paramter, 'rotation').on(
  'change',
  (v) => {
...
class App {
  ...
  resize() {
    this.canvas.width = window.innterWidth
